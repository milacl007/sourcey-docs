...
const CONVENTIONAL_SECTION_TYPES = {
    feat: "added",
    feature: "added",
    features: "added",
    "bug fix": "fixed",
...
export function normalizeChangelog(markdown, options = {}) {
    const diagnostics = [];
    ...
    const versionLinkMap = new Map(versionLinks.map((link) => [normalizeVersionKey(link.version), link.url]));
...
function newlineCount(input) {
    if (!input)
        return 0;
    return (input.match(/\n/g) ?? []).length;
}
function parseReferenceLinks(markdown) {
    const links = [];
    const seen = new Set();
...
function shouldContributeToDescription(token) {
    return token.type === "paragraph" || token.type === "blockquote" || token.type === "list";
}
function appendTokenToVersion(token, version, diagnostics, repoUrl) {
    if (token.type === "space")
        return;
...
function ensureTargetSection(version) {
...
function entriesFromToken(token, repoUrl) {
    if (token.type === "list") {
        const list = token;
...
function extractListItemMarkdown(item) {
...
function stripBlockquotePrefix(input) {
    return input
        .split("\n")
...
function createEntry(markdown, repoUrl) {
...
function extractRefs(markdown, repoUrl) {
    const refs = [];
    const seen = new Set();
    const repoBase = repoUrl?.replace(/\.git$/, "").replace(/\/$/, "");
    const pushRef = (type, id, url) => {
        const key = `${type}:${id}`;
        if (seen.has(key))
            return;
        seen.add(key);
    ...
    for (const match of markdown.matchAll(/(?:^|[^\w])#(\d+)/g)) {
        ...
        if (/(?:pr|pull request)\s*$/i.test(prefix))
            continue;
...
function normalizeVersionBlock(block, versionLinkMap) {
    return {
...
function validateDuplicates(versions, diagnostics) {
    const seen = new Map();
...
function validateSourceOrder(versions, diagnostics) {
    ...
    for (let i = 1; i < releaseVersions.length; i += 1) {
        ...
        if (!prev.version || !current.version)
            continue;
        if (compareSemverDesc(prev.version, current.version) > 0)
            continue;
        diagnostics.push({
            severity: "info",
...
function detectFormat(blocks) {
    ...
    for (const block of blocks) {
        for (const kind of block.rawSectionKinds)
            kinds.add(kind);
        if (/\]\([^)]*\)\s*\([^)]*\)/.test(block.rawHeading))
            conventionalHeading = true;
    }
    if (conventionalHeading || kinds.has("conventional"))
        return "conventional";
    if (kinds.has("standard") && !kinds.has("unknown") && !kinds.has("implicit"))
        return "keepachangelog";
    return "loose";
}
function resolveSectionKind(label) {
    const normalized = normalizeSectionLabel(label);
    if (STANDARD_SECTION_TYPES[normalized])
        return "standard";
    if (CONVENTIONAL_SECTION_TYPES[normalized])
        return "conventional";
    return "unknown";
}
function mapSectionType(label) {
    const normalized = normalizeSectionLabel(label);
...
function parseVersionHeading(text) {
    let value = text.trim();
    if (!value)
        return null;
    let yanked = false;
...
function buildParsedVersion(version, dateText, yanked) {
    return {
        version,
