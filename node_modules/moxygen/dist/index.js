...
function resolveOptions(options) {
    ...
    if ((opts.classes || opts.groups) && !opts.output.includes('%s')) {
        throw new Error("The `output` file parameter must contain '%s' for group or class name " +
...
function extractMeta(compound, description = extractRawDescription(compound)) {
    ...
    return {
        ...
        header: compound.includes,
        description,
    };
}
function extractResolvedMeta(compound, references, options, anchorMap, slugMap, pagePathMap) {
    const description = renderCompound(compound, [extractRawDescription(compound)], references, options, anchorMap, slugMap, pagePathMap);
...
function memberOwnerName(compound) {
    return compound.fullname || compound.name || qualifiedTitle(compound);
}
function collectOwnAllMemberEntries(compound) {
...
function collectInheritedMemberGroups(compound, references) {
    const seenBaseRefids = new Set();
    const groups = [];
    const visitBase = (baseRefid) => {
        if (!baseRefid || seenBaseRefids.has(baseRefid))
            return;
        seenBaseRefids.add(baseRefid);
        const base = references[baseRefid];
        if (!base || !('members' in base))
            return;
...
function attachRelationshipSummaries(compounds, references) {
    for (const compound of compounds) {
        compound.inheritedMemberGroups = collectInheritedMemberGroups(compound, references);
        const inheritedEntries = compound.inheritedMemberGroups.flatMap((group) => group.members.map((member) => ({
            name: member.name,
            kind: member.kind,
            refid: member.refid,
            owner: group.name,
...
function addGroupMarkerTags(source, marker, tags) {
    marker.lastIndex = 0;
    let match;
...
function collectSourceGroupTags(source) {
    const tags = new Set();
...
function resolveSourcePath(location, options) {
    const candidates = [];
    const seen = new Set();
...
function readFileGroupTags(file, options) {
    if (Array.isArray(file.fileGroupTags)) {
        return file.fileGroupTags;
    }
...
function markCompoundGroup(group, compound) {
    if (compound.groupid && compound.groupid !== group.id)
        return;
...
function attachCompoundToGroup(group, compound) {
    if (compound.groupid && compound.groupid !== group.id)
        return;
    group.compounds[compound.id] = compound;
    markCompoundGroup(group, compound);
}
function attachMemberToGroup(group, member) {
    if (member.groupid && member.groupid !== group.id)
        return;
    member.groupid = group.id;
    member.groupname = group.name;
...
function attachFileScopedNamespaceMembersToGroup(group, namespaceCompound, fileLocation) {
    let attached = false;
    ...
    if (attached) {
        const refs = group.fileScopedNamespaceRefs ?? [];
...
function pruneTopLevelGroupCompounds(group) {
    const topLevelRefids = new Set(Object.values(group.compounds).map((compound) => compound.refid));
...
function collectSharedNamespaceRefs(files, options) {
...
function augmentGroupsFromFiles(root, groups, options) {
    const files = toArray(root, 'compounds', 'file');
    if (!files.length)
        return;
    const sharedNamespaceRefs = collectSharedNamespaceRefs(files, options);
    const compoundsByRefid = new Map();
    for (const compound of toArray(root, 'compounds')) {
    ...
    for (const file of files) {
        ...
        for (const tag of tags) {
            ...
            if (fileLocation) {
                for (const refid of (file.fileNamespaceRefs ?? [])) {
                    if (!sharedNamespaceRefs.has(refid))
                        continue;
                    const candidate = compoundsByRefid.get(refid);
                    if (!candidate || candidate.kind !== 'namespace')
                        continue;
            ...
            for (const refid of (file.fileCompoundRefs ?? [])) {
                ...
                if (!candidate)
                    continue;
                if (isJunkCompound(candidate) || isNestedCompound(candidate))
                    continue;
                if (hasAncestorRefid(candidate, attachedNamespaceRefs))
                    continue;
                attachCompoundToGroup(group, candidate);
            }
...
function finalizeGroups(groups, sharedNamespaceRefs) {
    for (const group of groups) {
...
export async function generate(options) {
    ...
    const groups = toArray(root, 'compounds', 'group')
    ...
    if (useGroups) {
        ...
        finalizeGroups(groups, collectSharedNamespaceRefs(toArray(root, 'compounds', 'file'), opts));
        ...
        const allNamespaces = toArray(root, 'compounds', 'namespace')
            .filter((c) => !isJunkCompound(c));
        for (const ns of allNamespaces) {
            filterChildren(ns, opts.filters);
            for (const child of toFilteredArray(ns)) {
                if (seenPrep.has(child.refid) || isJunkCompound(child))
                    continue;
                filterChildren(child, opts.filters);
                prepareCompound(child);
                allCompounds.push(child);
        ...
        const rootCompounds = toArray(root, 'compounds', 'namespace')
            .filter((c) => !isJunkCompound(c));
    ...
    const seen = new Set();
    function emitPage(compound, moduleName) {
        if (seen.has(compound.refid))
            return;
        seen.add(compound.refid);
        if (isJunkCompound(compound))
            return;
    ...
    if (useGroups) {
        ...
        for (const ns of allNamespaces) {
            for (const child of toFilteredArray(ns)) {
                if (seen.has(child.refid) || isJunkCompound(child))
                    continue;
    ...
    for (const page of toArray(root, 'compounds', 'page')) {
        if (isJunkPage(page))
            continue;
...
function disambiguateDuplicatePageTitles(pages) {
...
function minimallyQualifiedTitles(pages) {
...
function commonNamespacePrefixLength(namespaces) {
    if (!namespaces.length || namespaces.some((parts) => parts.length === 0))
        return 0;
    let prefixLength = 0;
...
function collectSearchEntries(compound, separator, anchorMap) {
    const entries = [];
    const members = compound.filtered?.members?.length ? compound.filtered.members : compound.members;
...
function collectClassOutputCompounds(root) {
    return uniqueCompounds(toArray(root, 'compounds').filter(isClassOutputCompound));
}
function collectRootIndexCompounds(root, groups) {
...
function prepareRootIndex(root, groups, opts) {
    root.filtered.members = filterNoise(filterCollection(root.members, 'section', opts.filters.members));
    root.filtered.compounds = collectRootIndexCompounds(root, groups);
    root.filtered.sections = groupMembersBySection(root);
}
function ensurePathMap(pagePathMap) {
    return pagePathMap ?? new Map();
}
function addCompoundPath(pagePathMap, compound, opts) {
...
function definitionSectionTitle(compounds) {
    return compounds.every((compound) => TYPE_DEFINITION_KINDS.has(compound.kind))
        ? 'Class Definitions'
        : 'Definitions';
}
function renderCompoundPage(primary, definitions = []) {
    const contents = [
...
export async function run(options) {
    ...
    let rootBodyCompounds = [];
    let groups = [];
    let classOutputCompounds = [];
    if (opts.groups) {
        groups = toArray(root, 'compounds', 'group')
    ...
    if (opts.pages) {
        const doxyPages = toArray(root, 'compounds', 'page');
        if (!doxyPages.length) {
 ...
 */
function writeWithOptionalFrontmatter(compound, contents, references, options, anchorMap, pagePathMap) {
    if (options.frontmatter) {
        ...
        const fm = generateFrontmatter(extractResolvedMeta(compound, references, options, anchorMap, undefined, pagePathMap));
 ...
 */
function isJunkCompound(compound) {
    const name = compound.name;
    if (JUNK_NAMESPACES.has(name))
        return true;
    if (JUNK_NAME_RE.test(name))
        return true;
    if (name === 'deprecated')
        return true;
    return false;
...
function findNamespace(compound) {
    let current = compound.parent;
    while (current) {
 ...
 */
function qualifiedTitle(compound) {
