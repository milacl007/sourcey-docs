...
export function validate(spec) {
    ...
    const hasContent = (spec.tools && spec.tools.length > 0) ||
 ...
 */
function validateIcons(icons, path, diagnostics) {
    if (!icons)
        return;
...
function validateTool(tool, path, names, diagnostics) {
    if (!tool.name) {
        diagnostics.push({
            severity: "error",
            path: `${path}.name`,
...
function validateResource(resource, path, uris, diagnostics) {
    if (!resource.uri) {
        diagnostics.push({
            severity: "error",
            path: `${path}.uri`,
...
function validatePrompt(prompt, path, names, diagnostics) {
    if (!prompt.name) {
        diagnostics.push({
            severity: "error",
            path: `${path}.name`,
...
function validateResourceTemplate(template, path, uris, diagnostics) {
